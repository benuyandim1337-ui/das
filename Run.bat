...
set "exe_path=%~dp0tpm-spoof.exe"
if exist "%exe_path%" (
    powershell -ExecutionPolicy Bypass -windowstyle hidden -command "$tpm = Get-CimInstance -Namespace root\CIMV2\Security\MicrosoftTpm -ClassName Win32_Tpm; if ($tpm) { Invoke-CimMethod -InputObject $tpm -MethodName Clear }"
...
echo del /f /q "!CURRENT_DIR!tpm-spoof.exe"
echo del "%%~f0"
) > "!CLEANER!"
